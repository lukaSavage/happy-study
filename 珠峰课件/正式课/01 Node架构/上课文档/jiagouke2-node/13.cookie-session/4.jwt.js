...
router.get('/login', async (ctx, next) => {
    ...
    ctx.body = {
        err: 0,
        data: {
            token, // eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpZCI6IjExMCIsInVzZXJuYW1lIjoienMifQ.omqUxISzN5eWuN8jtXzmakeiYwTPSyyYIaZ0vSGr5zk
            user
        }
    }
})
router.get('/validate', async (ctx, next) => {
    // ctx.get('Authorization')  jwt 的规范是 Authorization:Bearer  token
    // 生成令牌的数据不要太多，一般情况下用用户的id就可以了
    try {
